...
### 反映の記録（2026-09-21・Production 反映済み）

Firebase コンソールから手作業で反映。反映の前後で、未ログイン状態の
Firestore REST API（`firestore.googleapis.com`／フロントと同じ公開APIキー）に
実際に読みに行って確かめた。

| コレクション | 反映前 | 反映後 | 意味 |
|---|---|---|---|
| `sp_auth_links` | 200（読めた） | **403** | 認証情報→番号の対応表。閉じた |
| `sp_identities` | 200（読めた） | **403** | 閉じた |
| `sp_link_tickets` | 200（読めた） | **403** | 連携の引換券。閉じた |
| `sp_identity_duplicates` | 200（読めた） | **403** | 閉じた |
| `sp_identity_audit` | 200（読めた） | **403** | 閉じた |
| `sp_members` | 200（読めた） | **403** | 未使用の旧コレクション。閉じた |
| `paid_users` | 200 | **200** | `allow read: if true`。変化なし（正常） |
| `posts` | 200 | **200** | 変化なし |
| `user_profiles` | 200 | **200** | 変化なし |
| `emuer_offchain` | 200 | **200** | 変化なし |
| `airdrop_registrations` | 200 | **200** | 変化なし |
| `sp_quests` | 403 | **403** | 変化なし |
| `ches_accounts` | 403 | **403** | 変化なし |

反映前の時点で新規5コレクションが誰でも読める状態だったのは、
`match /{coll}/{document=**}` の汎用許可に落ちていたため。
コレクション自体が空だったので実害は無い。

書き込みの検証は行っていない。読み取りが 403 を返す時点でルールは反映済みと
確かめられており、書き込みを試すと（万一ルールが未反映だった場合に）
Production に不要な文書を作ってしまうため。

なお、反映前の Production のルールは、リポジトリの内容と
`sp_members` の1行と空行1つを除いて完全に一致していた（全1315行を照合）。
`sp_members` はコレクションごと手作業で削除済みで、コードからの参照も0件。
