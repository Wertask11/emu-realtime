...
async function wsRewards(){
  try{const r=await fetch(EMUER_V2_API+"/rewards?address="+encodeURIComponent(wsAccount()),{headers:await wsHeaders(false)}),d=await r.json(),a=r.ok&&Array.isArray(d.rewards)?d.rewards:[];if(!a.length){wsBtn().hidden=true;return;}wsClaimLabel(`未請求の報酬 ${a[0].amount} EMUER を受け取る（全${a.length}件）`,()=>wsClaim(a[0]),false);}catch(_){}
}
